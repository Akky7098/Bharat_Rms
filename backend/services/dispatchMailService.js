...
/* =========================================================
   GET ALL TC / MTC FILES

   Supports:

   NEW:
   dispatch.tcCertificatePdfs[]

   LEGACY:
   dispatch.tcCertificatePdf

   Important:
   Avoid attaching the same first TC twice because
   createDispatch keeps the first file in both fields
   temporarily for backward compatibility.
========================================================= */

const getTcCertificateFiles = (
  dispatch
) => {
  if (
    dispatch?.tcApplicable !==
    "applicable"
  ) {
    return [];
  }
  ...
  /*
   * LEGACY single-TC fallback.
   *
   * Also safe when new array exists because duplicate
...
const getPaymentAttachments = (
  ...
  if (
    ...
    attachments.push({
      ...
      path:
        paymentBillPdf.filePath,

      contentType:
        paymentBillPdf.mimeType ||
        "application/pdf",
    });
  }
...
const getDispatchAttachments = (
  ...
  /* =====================================================
     ALL TC / MTC CERTIFICATES
  ===================================================== */

  const tcFiles =
    getTcCertificateFiles(
      dispatch
    );
...
const getDispatchAttachmentNames = (
  dispatch
) => {
  const names = [];
  ...
  if (
    ...
    names.push(
      `${tcFiles.length} MTC / TC Certificates`
    );
  }
...
const getReadableAttachmentText = (
  dispatch
) => {
  const names =
    getDispatchAttachmentNames(
      dispatch
    );
...
/* =========================================================
   ...
   Applicable — 1 Certificate Attached

   Applicable — 4 Certificates Attached

   Applicable — Certificate Unavailable
========================================================= */

const getTcStatusText = (
  dispatch
) => {
  if (
    dispatch?.tcApplicable !==
  ...
  if (
    ...
    return "Applicable — 1 Certificate Attached";
  }

  if (
    tcFiles.length > 1
  ) {
    return `Applicable — ${tcFiles.length} Certificates Attached`;
  }
              ...
              >
                ...
                ${
                  badge
                    ? `
                      <tr
                        class="mobile-badge-row"
                        style="display:none;"
                      >
                        <td
                          class="mobile-badge-cell"
                          colspan="3"
                          style="display:none;"
                        >
                          <span
          ...
          <tr>
            ...
            >
              ...
              <div
                ...
                "
              >
                This is an automated notification from Bharat RMS.
...
const buildDispatchCreatedTemplate = (
  dispatch
) => {
  const attachmentText =
    getReadableAttachmentText(
      dispatch
    );
  ...
  return baseEmailTemplate({
    preHeader:
      `Material dispatched. Invoice ${dispatch.invoiceNumber}.`,
...
/* =========================================================
   PAYMENT UPDATE TEMPLATE
========================================================= */

const buildPaymentUpdateTemplate = (
  dispatch,
  payment
) => {
  const isFullyPaid =
  ...
  return baseEmailTemplate({
    preHeader:
      `Payment received for invoice ${dispatch.invoiceNumber}.`,
...
/* =========================================================
   SEND DISPATCH CREATED EMAIL

   All available TC/MTC files are attached here.
========================================================= */

const sendDispatchCreatedEmail =
  async (
    dispatch
  ) => {
    const to =
    ...
    const mail =
      await transporter.sendMail({
        from:
          `"${COMPANY.name}" <${process.env.ADMIN_EMAIL}>`,

        to,

        subject:
          `Dispatch Confirmation | Invoice ${
            dispatch.invoiceNumber
          } | ${
            dispatch.companyName
          }`,
