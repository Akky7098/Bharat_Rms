...
const sendWhatsAppText = async (message) => {
  ...
  return client.sendMessage(WHATSAPP_GROUP_ID, message);
};
...
const getTodayData = async () => {
  ...
  const [employees, attendance] = await Promise.all([
    User.find({
      ...
      .select("_id name email role attendanceWorkMode attendanceMode")
      .lean(),
  ...
  attendance.forEach((item) => {
    attendanceMap.set(normalizeId(item.employeeId), item);
  });

  const rows = employees
    .map((emp) => {
      const record = attendanceMap.get(normalizeId(emp._id));
...
const getEmployeeMode = (employee, attendance) => {
  const mode =
    attendance?.workMode ||
  ...
  return mode === "work_from_home" ? "WFH" : "Office";
};

const buildMorningWhatsAppMessage = ({ date, rows }) => {
  const present = rows.filter((r) => r.attendance?.checkIn?.time);
  const absent = rows.filter((r) => !r.attendance?.checkIn?.time);

  const office = present.filter(
    (r) => getEmployeeMode(r.employee, r.attendance) === "Office"
  );

  const wfh = present.filter(
  ...
  const line = (r) => {
    ...
    return `• ${name} — ${formatTime(r.attendance?.checkIn?.time)}`;
  };
...
🏢 *Office*
${office.length ? office.map(line).join("\n") : "No office check-ins yet"}
...
const buildEmailHtml = ({ title, subtitle, rows, type }) => {
  const rowHtml = rows
    .map(({ employee, attendance }) => {
      const mode = getEmployeeMode(employee, attendance);
      ...
      return `
        <tr>
          ...
          <td style="padding:10px;border-bottom:1px solid #e5e7eb;">${mode}</td>
          <td style="padding:10px;border-bottom:1px solid #e5e7eb;">${formatTime(
            attendance?.checkIn?.time
          )}</td>
          <td style="padding:10px;border-bottom:1px solid #e5e7eb;">${formatTime(
            attendance?.checkOut?.time
          )}</td>
          <td style="padding:10px;border-bottom:1px solid #e5e7eb;font-weight:800;">${formatMinutes(
            minutes
          )}</td>
...
<body style="margin:0;padding:0;background:#f4f7fb;font-family:Arial,Helvetica,sans-serif;color:#111827;">
  <table width="100%" cellpadding="0" cellspacing="0" style="padding:20px 0;background:#f4f7fb;">
    <tr>
      <td align="center">
        <table width="760" cellpadding="0" cellspacing="0" style="width:96%;max-width:760px;background:#ffffff;border:1px solid #e5e7eb;border-radius:18px;overflow:hidden;">
          <tr>
            <td style="padding:18px 22px;background:#0f172a;color:#ffffff;">
              <div style="font-size:18px;font-weight:900;">Bharat Special Steels</div>
              <div style="font-size:12px;color:#cbd5e1;margin-top:3px;">Attendance ${type} Report</div>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:22px;">
              ...
              <p style="margin:7px 0 0;font-size:14px;color:#64748b;">${subtitle}</p>
            </td>
          </tr>
            ...
            <td style="padding:0 22px 24px;">
              <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e5e7eb;border-radius:14px;overflow:hidden;font-size:13px;">
                <tr style="background:#f8fafc;">
                  <th align="left" style="padding:10px;border-bottom:1px solid #e5e7eb;">Employee</th>
                  <th align="left" style="padding:10px;border-bottom:1px solid #e5e7eb;">Mode</th>
                  <th align="left" style="padding:10px;border-bottom:1px solid #e5e7eb;">Check In</th>
                  <th align="left" style="padding:10px;border-bottom:1px solid #e5e7eb;">Check Out</th>
                  <th align="left" style="padding:10px;border-bottom:1px solid #e5e7eb;">Total</th>
...
const sendEmailReport = async ({ title, subtitle, rows, type }) => {
  ...
  return transporter.sendMail({
    ...
    subject: `${title} | Bharat RMS`,
    html: buildEmailHtml({ title, subtitle, rows, type }),
  });
};

const sendMorningAttendanceDigest = async () => {
  const data = await getTodayData();

  await sendWhatsAppText(buildMorningWhatsAppMessage(data));

  await sendEmailReport({
    title: "Morning Attendance Snapshot",
    subtitle: `${formatDate(data.date)} · 10:15 AM`,
    rows: data.rows,
    type: "Morning",
  });
...
const sendEveningAttendanceDigest = async () => {
  ...
  await sendEmailReport({
    ...
    subtitle: `${formatDate(data.date)} · 7:00 PM`,
    rows: data.rows,
    type: "Evening",
  });
